...
export const POST = async (req: Request) => {
  ...
  try {
    ...
    const product = await prisma.$transaction(async (prisma) => {
      ...
      const isProductAlreadyInCart = await checkIsProductAlreadyInCart({
        cart_id,
        product_id,
      });
...
export const DELETE = async (req: Request) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ message: "Product removed successfully", result }),
      { status: 200 }
    );
  } catch (error) {
    const message = (error as Error).message;
    ...
    return new Response(
      JSON.stringify({ message: message || "Unable to delete product" }),
      { status: 400 }
    );
  }
};
...
export const PATCH = async (req: Request) => {
  ...
  try {
    ...
    const isProductInCart = await checkIsProductAlreadyInCart({
      cart_id,
      product_id,
    });
    ...
    const message = (error as Error).message;
    return new Response(
      JSON.stringify({ message: `Something went wrong: ${message}` }),
      { status: 500 }
    );
  }
};
